...
namespace AdvancedCustomBlips
{
    ...
    {
        private const string _MOD_PAGE_URL = "https://www.gta5-mods.com/scripts/advanced-custom-blips-fe7d9d03-1fc1-4088-9fae-005dc2b1a270";
        private const string _VERSION_REGEX_PATTERN = @"Advanced Custom Blips\s+v(\d+\.\d+\.\d+)";
        private readonly string _currentScriptVersion = "v4.3.1";

        private const string _CREATE_GROUP_LABEL = "~y~Create New Group";
        private const int _DEFAULT_INTERVAL = 1000;
        private const int _ACTIVE_INTERVAL = 1;
        private const int _TYPE_SPEED = 25; 
        ...
        private readonly string _iniBackupPath = Path.Combine("scripts", "Advanced Custom Blips.ini.backup");
        ...
        private const int _BLIPS_PER_TICK = 10; 
...
new IconEntry((ExtendedBlipSprite)950, 516, 585),
        ...
        public Main()
        {
            ...
            _availableGroups.Add(_CREATE_GROUP_LABEL);
        ...
        private async Task InitializeAsync()
        {
            ...
            _areBlipsVisible = _globalSettings.AreBlipsVisible;

            _mainMenu = CreateAndRegisterMenu("Advanced Custom Blips", $"Main Menu {_currentScriptVersion}");
            ...
            _customSprite = new CustomSprite(
                LoadTempfile("AdvancedCustomBlips.Assets.VanillaTextureSheet.image.png"),
                new SizeF(640f, 360f),
                new PointF(Screen.Width * 0.49f, 5)
            );

            _customSprite1 = new CustomSprite(
                LoadTempfile("AdvancedCustomBlips.Assets.VanillaTextureSheet.image1.png"),
                new SizeF(640f, 350f),
                new PointF(Screen.Width * 0.49f, Screen.Height * 0.51f)
            );

            _customSprite2 = new CustomSprite(
                LoadTempfile("AdvancedCustomBlips.Assets.VanillaTextureSheet.image2.png"),
                new SizeF(520f, 230f),
                new PointF(Screen.Width * 0.01f, Screen.Height * 0.56f)
            );

            _mousePointer = new CustomSprite(
                LoadTempfile("AdvancedCustomBlips.Assets.VanillaTextureSheet.Mouse Pointer.png"),
            ...
            for (int a = 0; a < _availableGroups.Count; a++)
            {
                ...
                for (int c = 0; c < groupBlips.Count; c++)
                {
                    ...
                    NativeListItem<string> groupItem = new NativeListItem<string>("Current Group:", groups)
                    {
                        ...
                        Description = $"Assign this blip to an existing group or create a new one. To create a new group switch list until it shows \"{_CREATE_GROUP_LABEL}~w~\" then select it and you will be able to type the group name."
                    };
                    ...
                    saveBtnItem.Activated += async (menu, item) =>
                    {
                        ...
                        bool isCreateGroupLabel = currentLabel == _CREATE_GROUP_LABEL;
        ...
        private void OnTick(object sender, EventArgs e)
        {
            ...
            Interval = (_uiMenuPool.AreAnyVisible ||                            
                        ...
                        ? _ACTIVE_INTERVAL : _DEFAULT_INTERVAL;

            if (_blipQueue.Count > 0)
            {
                int count = Math.Min(_BLIPS_PER_TICK, _blipQueue.Count);
        ...
        private async void OnKeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (keyName == _globalSettings.ToggleBlipsVisibilityKey)
            {
                ...
                _ = SaveBlipsVisibilitySettingAsync();
            }
        ...
        private void InitializeGlobalSettingsMenu()
        {
            ...
            saveSettingsBtn.Activated += async (sourceMenu, clickedItem) =>
            {
                ...
                _globalSettings.AreBlipsVisible = _areBlipsVisible;
        ...
        private string GetTypingText()
        {
            ...
            if (Game.GameTime - _lastTypeTime >= _TYPE_SPEED)
            {
                _lastTypeTime = Game.GameTime;
                _typingIndex++;
        ...
        private async Task SaveBlipsVisibilitySettingAsync()
        {
            if (!File.Exists(_settingsFilePath))
                return;
            await _fileLock.WaitAsync();
            ...
            {
                DisplayAndLogError(ex, "Error saving blips visibility status.");
            }
            finally
            {
                _fileLock.Release();
        ...
        private void CreateBlipFromData(BlipData data)
        {
            ...
            if (!_areBlipsVisible)
            {
                Function.Call(Hash.SET_BLIP_DISPLAY, blip.Handle, 0);
            }
        ...
        private async Task CheckForUpdatesAsync()
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    string htmlContent = await client.GetStringAsync(_MOD_PAGE_URL);

                    
                    Match match = Regex.Match(htmlContent, _VERSION_REGEX_PATTERN, RegexOptions.IgnoreCase);
                    if (match.Success && match.Groups.Count > 1)
                    {
                        string remoteVersion = "v" + match.Groups[1].Value; 

                        
                        int versionComparison = string.Compare(_currentScriptVersion, remoteVersion, StringComparison.OrdinalIgnoreCase);
                        ...
                        {
                            Notification.PostTicker($"~g~[Advanced Custom Blips] Up to date. Version: {_currentScriptVersion}", true);
                        }
                    }
                    else
                    {
    ...
    public sealed class SettingsData
    {
        ...
        public bool AreBlipsVisible { get; set; } = true;
    }
    ...
    public enum ExtendedBlipSprite  
    {
        ...
        radar_heist_art_unavailable = 958,
        radar_heist_art = 959,
        radar_property_mansion_art_heist = 960,
        radar_art_heist_prep = 961,
        radar_manhole_key = 962,
        radar_buyers_request = 963,
        radar_armored_caracara = 964,
        radar_witness = 965
    }
